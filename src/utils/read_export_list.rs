...
/// Read export list and return a HashMap of `source val name : dest val name``
...
mod tests {
    ...
    fn test_read_export_list_succeed() {
        ...
        expected.insert("number_float".to_string(), "number_float_copy".to_string());
        expected.insert(
    ...
    fn test_read_export_list_fail() {
        let path = "files/test/export_list.noline.txt";
        let resp = read_export_list(&path).unwrap();
        assert_eq!(resp, None);
    }
}
